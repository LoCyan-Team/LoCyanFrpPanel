...
const DoDonate = () => {
  ...
  const rs = get("https://api.locyanfrp.cn/Pay?money=" + amount.value + "&name=LoCyanFrp Donate&username=" + store.getters.GetUserName + "&type=" + pay_type.value + "&return_url=http://localhost:5173/donate", []);
  rs.then(res => {
    if (res.status === true){
      window.open(res.url);
      loading_donate.value = false;
