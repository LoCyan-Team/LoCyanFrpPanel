    ...
    <n-grid-item span="1">
      <n-form ref="formRef" :model="model" :rules="rules" label-width="auto" require-mark-placement="right-hanging"
        ...
        <div style="display: flex; justify-content: flex-end">
          <n-space>
            <n-button type="primary" @click="sendresetmail" style="margin-right: 10px;"> 发送重置邮件 </n-button>
      ...
      <n-form ref="formRef" :model="reset_password" :rules="rules" label-width="auto"
        ...
        <div style="display: flex; justify-content: flex-end">
          <n-space>
            <n-button type="primary" @click="doresetrequest" style="margin-right: 10px;"> 提交重置请求 </n-button>
          </n-space>
        </div>
      </n-form>
    </n-grid-item>
...
function doresetrequest() {
  ...
  ldb.start();
  const rs = get("https://api.locyanfrp.cn/User/DoResetPassword?password=" + reset_password.value.password + "&confirm=" + reset_password.value.confirm + "&code=" + code, []);
  rs.then(res => {
    if (res.status === true) {
...
function sendresetmail() {
  ...
  ldb.start();
  const rs = get("https://api.locyanfrp.cn/User/DoResetPassowrdEmailSend?username=" + model.value.username, []);
  rs.then(res => {
    if (res.status === true) {
      message.success(res.message);
    } else {
